...
- **卡組清單「時間軸」視圖**（`SetTimeline`，右上角 toggle 切換卡片牆／時間軸）
  - 中央一條時間線，卡組依發行日左右交錯掛在線上，用箭頭指回軸線；顏色本身就是圖例——每個產品線一個顏色（`constants/productTypes.ts` 的 `PRODUCT_TYPE_THEME`），節點、邊條、箭頭、進度條同色
  - 每個卡組除了卡片牆原有的資訊（卡號、產品線、日期、名稱、收集進度），再加上**貴罕度分布**（最稀有在前，只列 5 種其餘收成 `+N`）、**該卡組最稀有的 4 張卡圖**與**卡組包裝圖**
  - 卡圖取「該卡最稀有的那個印刷」，所以同一張卡不會因為有 N/SR/SER 三種版本就佔掉三格；不足 4 張時留空位，盒子高度才不會忽高忽低
  - **點縮圖放大**：卡圖與卡組圖片都可點開燈箱（標題顯示卡名 + 卡號 · 貴罕度），左右鍵或兩側箭頭在同一個卡組的圖片間移動。卡組圖片不只一張時，開燈箱才去補齊整個 gallery（補完卡圖索引跟著挪，不會跳到別張）
  - 盒子本身仍整塊可點進卡組頁：連結鋪滿盒子墊在底層，圖片疊在它上面自己吃點擊；`.tl-row` 設 `pointer-events: none`（列與列上下重疊，否則後面那列的空白區會蓋住前一列盒子的下半部，讓那裡點不到）
  - 左右交錯本來就空出垂直空間：盒高固定 228px、列距 130px，相鄰兩列（異側）緊密咬合，同側（相隔一列）仍留 32px 淨空
  - **沒有發行日的卡組不會出現**（目前 17 個）——時間軸上沒有它們的位置，要看請切回卡片牆
  - 版面寬度不足 1100px 時放棄交錯：軸線靠左、卡組一律排右邊、盒高改為自動
  - 後端 `GET /api/card-sets/timeline`（`services/timeline_service.py`）：統計、貴罕度分布、高貴罕度卡圖、包裝圖一次拿齊，避免 4 × 90 次請求。**路由必須定義在 `/{set_id}` 之前**，否則會被 catch-all 攔截
...
- **卡組記住自己的 yugipedia 頁面，並顯示卡組圖片**
  ...
  - 卡組清單 hover 時顯示該卡組的圖片（浮動視窗，固定高度 150px）；每個卡組只查一次 API 就快取，進場延遲 120ms 避免滑過一整排時連打，沒有圖就不彈視窗
  - 對照卡表對話框移到卡組載入狀態的分支外面：存網址後會重新載入卡組，對話框若在裡面會連同比對結果一起被卸載
