...
**存檔時的合併**（`scraper._merge_with_existing()`）：一篇文章可能順帶提到別的 set 的卡
（補充包文章引用上一彈的一張卡），或同一個促銷系列散在好幾篇（`S23P`）。所以磁碟上已有
同名 set 且**來自別篇文章**時，這次只能**增加**沒見過的卡，不覆寫既有卡片與 metadata；
同一篇重爬才是整份取代。少了這道保護，一次順帶引用就會把 70 張的 set 蓋成 1 張，
接著 `_cleanup_orphaned_images()` 會把其餘卡圖刪掉。
