...
const canApply = computed(() => missingChecked.value.length + extraChecked.value.length > 0)

function toggleAll(which: 'missing' | 'extra') {
  ...
  >
    <div class="flex flex-col gap-4">
      ...
      <template v-if="result">
        ...
        <div
          v-if="!result.missing.length && !result.extra.length"
    ...
    <template #footer>
      <div class="flex items-center justify-between gap-3 w-full">
        <span v-if="result && canApply" class="text-[11px] text-gray-400">
          將新增 {{ missingChecked.length }} 種、刪除 {{ extraChecked.length }} 種
        </span>
        <span v-else />
