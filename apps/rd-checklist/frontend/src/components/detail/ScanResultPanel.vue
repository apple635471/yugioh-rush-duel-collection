...
// Initial position: to the left of the sidebar (fixed at right edge - sidebar width - panel width - gap)
const x = ref(window.innerWidth - 448 - 360 - 16)
const y = ref(80)
...
function onMouseMove(e: MouseEvent) {
  ...
  x.value = Math.max(0, Math.min(window.innerWidth - 360, e.clientX - dragOffsetX.value))
...
async function copyAll() {
  ...
  const lines: string[] = []
  for (const { key, label } of fields.value) {
    const val = (props.result as any)[key]
    if (val != null && val !== '') {
...
const fields = computed(() => {
  ...
  const isMonster = r?.card_type?.includes('怪獸') ?? true
  return [
    { key: 'name_zh',          label: '卡名（中）',   show: true },
    { key: 'name_jp',          label: '卡名（日）',   show: true },
    { key: 'card_type',        label: '卡牌種類',      show: true },
    { key: 'is_legend',        label: 'LEGEND',        show: true },
    { key: 'attribute',        label: '屬性',          show: isMonster },
    { key: 'monster_type',     label: '種族',          show: isMonster },
    { key: 'level',            label: '等級',          show: isMonster },
    { key: 'atk',              label: 'ATK',           show: isMonster },
    { key: 'defense',          label: 'DEF',           show: isMonster },
    { key: 'description',      label: 'Description',   show: true },
    { key: 'summon_condition', label: 'Summon Cond.',  show: isMonster },
    { key: 'condition',        label: 'Condition',     show: true },
    { key: 'effect',           label: 'Effect',        show: true },
    { key: 'continuous_effect',label: 'Cont. Effect',  show: true },
  ].filter(f => f.show)
})
...
<template>
  <Teleport to="body">
    <div
      class="fixed z-[90] w-[360px] bg-gray-800 border border-gray-600 rounded-xl shadow-2xl flex flex-col select-none"
    ...
    >
      ...
      <!-- Body -->
      <div class="overflow-y-auto max-h-[70vh] px-4 py-3 space-y-1.5">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-10 gap-3">
          ...
          <span class="text-sm text-gray-400">呼叫 gpt-4o 中…</span>
        </div>
        ...
        <template v-else-if="result">
          ...
          >
            <!-- Label -->
            <span class="w-24 shrink-0 text-xs text-gray-500 pt-0.5">{{ field.label }}</span>

            <!-- Value -->
            <span
              class="flex-1 text-sm leading-relaxed whitespace-pre-line break-words"
              :class="hasValue((result as any)[field.key]) ? 'text-gray-200' : 'text-gray-600'"
            >
              {{ displayValue(field.key, (result as any)[field.key]) }}
            </span>

            <!-- Copy button (show on hover, only if has value) -->
            <button
              v-if="hasValue((result as any)[field.key])"
              @click="copyField(field.key, displayValue(field.key, (result as any)[field.key]))"
              class="shrink-0 p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity text-gray-500 hover:text-yellow-400"
              title="複製"
            >
              <!-- Check on copied, clipboard otherwise -->
              <svg v-if="copiedKey === field.key" class="w-3.5 h-3.5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
